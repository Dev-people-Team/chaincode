...
infoln "TEST1 : Mint token"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"mint","Args":[]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3

infoln "TEST2 : Check Client"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"clientAccountID","Args":[]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function":"clientAccountBalance","Args":[]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3